...
class _PointHistory:
    ...
    def __init__(self, maxlen: int, alpha: float):
        ...
        # We store the raw incoming points, up to maxlen. 
        # (Could be useful if we need median filtering later).
        self.raw_buffer: collections.deque[Optional[Point]] = collections.deque(maxlen=maxlen)
        
        # EMA smoothed point
        self.ema_pos: Optional[Point] = None
        
        # Last known valid EMA to calculate derivatives
    ...
    def update(self, pt: Optional[Point]) -> None:
        ...
        # Update EMA
        if pt is None:
            # If point is missing, we don't update the EMA with a None.
            # We simply keep the old EMA (decaying velocity to 0 conceptually)
            # or we could set EMA to None. To prevent corruption and teleportation 
            # when it reappears, we leave EMA as is, but velocity becomes 0.
        ...
        if self.ema_pos is None:
            ...
            self.ema_pos = (
                self.ema_pos[0] * (1 - self.alpha) + pt[0] * self.alpha,
                self.ema_pos[1] * (1 - self.alpha) + pt[1] * self.alpha,
            )
...
class _FighterTemporalState:
    ...
    def __init__(self, maxlen: int, alpha: float):
        self.head = _PointHistory(maxlen, alpha)
        self.body = _PointHistory(maxlen, alpha)
        self.l_wrist = _PointHistory(maxlen, alpha)
        self.r_wrist = _PointHistory(maxlen, alpha)
        self.l_elbow = _PointHistory(maxlen, alpha)
        self.r_elbow = _PointHistory(maxlen, alpha)
        self.l_ankle = _PointHistory(maxlen, alpha)
        self.r_ankle = _PointHistory(maxlen, alpha)
...
class TemporalFeatureManager:
    ...
    def update(self, features_list: list[PoseFeatures]) -> dict[int, SmoothedFeatures]:
        ...
        if len(valid_feats) == 2:
            ...
            if dist is not None:
                # Basic EMA for scalar distance
                if self.fighter_distance is None:
                    self.fighter_distance = dist
                else:
                    self.fighter_distance = self.fighter_distance * (1 - self.alpha) + dist * self.alpha
        else:
            dist = None
